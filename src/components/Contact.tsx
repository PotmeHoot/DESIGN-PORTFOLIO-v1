...
export const Contact = () => {
  ...
  return (
    <SectionWrapper containerClassName="max-w-5xl text-center">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-5xl aspect-square bg-accent/[0.02] rounded-full blur-[120px] -z-10" />
      ...
      >
        ...
        <h2 className="section-title">
          ...
          {contact.subtitle} <span className="text-text-muted">{contact.highlight}</span>
        </h2>
        ...
        <div className="flex flex-col items-center gap-16 md:gap-24">
          ...
          <div className="grid md:grid-cols-2 gap-12 md:gap-32 items-start text-left md:text-center w-full max-w-3xl mx-auto">
            <div className="space-y-4">
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-text-muted">{contact.directLabel}</span>
              <a href={`mailto:${settings.email}`} className="block text-xl md:text-2xl font-bold hover:text-accent transition-colors">
                {settings.email}
              </a>
            </div>
 
            <div className="space-y-4">
              <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-text-muted">{contact.socialLabel}</span>
              <div className="flex flex-wrap justify-start md:justify-center gap-8">
                {contact.socials.map((social) => (
                  <a 
                    ...
                    className="text-sm md:text-base font-bold text-text-secondary hover:text-accent transition-colors uppercase tracking-widest"
                  >
                    {social.name}
                  </a>
                ))}
